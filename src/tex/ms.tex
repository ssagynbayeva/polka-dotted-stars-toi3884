...
\begin{abstract}
    ...
    \chreplaced[id=W]{change}{bump} in the light curve that directly reveals
    ...
    \chadded[id=W]{surface map} representation from \texttt{starry}, the
    probabilistic approach \chadded[id=W]{to spot properties} of
    \texttt{StarryProcess}, and a comprehensive transit model that accounts for
    spot-crossing events \chadded[id=W]{during transits}. We demonstrate through
    ...
    disentangling stellar activity \chcomment[id=W]{Here it seems that you are using ``stellar activity'' as a synonym for ``stellar spots,'' but I don't think that's the case?} from planetary signals while simultaneously
...
variability. Recent JWST results on TRAPPIST-1b \citep{Lim2023} demonstrated that accurate measurements have been \chdeleted[id=W,comment={I think you mean to say that the spot contamination pretty much destroyed the measurement, but ``substantially hindered'' sounds weird to me}]{substantially} 
hindered by stellar contamination, highlighting the need for \chreplaced[id=W,comment={it's not that the model needs to be more ``precise'' it's that existing models just don't account for this properly/reasonably}]{better}{more precise} models to account for these effects. To fully leverage 
...
represent \chadded[id=W]{each object's} surface brightness distribution,
...
\chreplaced[id=W]{uses a model for the}{treats} spot distribution
\chreplaced[id=W]{to derive}{as} a Gaussian process \chadded[id=W]{for the
...
\begin{itemize}
    ...
    \item \chadded[id=W]{The capability to model evolving stellar surfaces
...
distribution patterns \chadded[id=W]{as well as providing constraints on the
...
\chreplaced[id=W]{without the presence of a transit.}{in conventional analysis.}

Our paper is organized as follows: Section \ref{sec:model} presents the
\texttt{StarryStarryProcess} model; \chcomment[id=W]{What about Section 3
alone?} Section \ref{sec:experiment1} presents results on non-evolving stellar
surfaces; Section \ref{sec:experiment2} presents results on evolving surfaces;
...
\chcomment[id=W]{I have made a lot of changes to this section, and eventually gave up tracking them as comments; if you want to track them, you will have to git diff against the earlier version.}

\chcomment[id=W]{Can you go through and make sure I've fixed all the
notation---it should be $\mathbb{f}_\mathrm{true}$ (blackboard-bold and also
Roman subscript) throughout, $\pmb{\Theta}$ (bold), etc.}
...
\label{sec:obl-inc}
In the Sectrion \ref{sec:results-synthetic}, we \chreplaced[id=W]{demonstrate
that}{hope to convince the reader that} there is a a good amount of information
about stars that we can learn from disk-integrated photometric measurements
\chadded[id=W]{in the presence of transits}. In particular, Figure
...
variations. This is because the spots remain invisible \chadded[id=W]{or
...
the variations typically increases, and the shape \chadded[id=W]{changes} of the
light curve become\chdeleted[id=W]{s} more pronounced. These changes occur
...
\chreplaced[id=W]{A}{Specifically, a} system with stellar inclination $i_\star$
...
map \chreplaced[id=W]{reflected about the equator}{flipped upside down}.
...
\chadded[id=W]{non-degenerate part of the} parameter space effectively
\chreplaced[id=W]{as described below}{while respecting physical constraints}.

The transformation from these sampled variables to the physical space \chreplaced[id=W]{begins with}{is then defined as}:
\begin{eqnarray}
    x & = & \tilde{x}, \\ 
...
From these transformed \chadded[id=W]{cartesian} coordinates, we can derive the
stellar inclination and obliquity:
\begin{eqnarray}
    i_\star & = & \arctan{\frac{y}{x}}, \\ 
    \lambda_\star & = & \arccos{\frac{z}{\sqrt{x^2 + y^2 + z^2}}}
\end{eqnarray}
\chadded[id=W]{With these definitions, the degeneracy identified above
...
\chcomment[id=W]{Shouldn't these $\pmb{y}$ actually be $\mathbb{y}$?  And why
are the times $\pmb{t}$ written in bold?}  Specifically, let $\pmb{y}_1$,
...
A key aspect is that each pair of consecutive maps $\pmb{y}_i$, $\pmb{y}_{i+1}$
is interpolated independently from other map pairs. Thus, $\pmb{y}_{i+1}$ does
not depend on $\pmb{y}_{i-1}$, and $\pmb{y}_{i+2}$ is independent of
$\pmb{y}_{i+1}$, allowing the model greater flexibility to fit arbitrary time
variations. The framework developed for static maps in Section \ref{sec:model}
is applied independently to each interpolated map $\pmb{y(t)}$, with the
transition between maps governed by the linear interpolation above.
\chadded[id=W]{The predicted true flux remains linear in the maps; the complete
...
\chreplaced[id=W]{epochs}{chunks} of the \chadded[id=W]{interpolated} light
curve span times \chreplaced[id=W]{that are comparable to the spot evolution
timescale.}{that's $\leq$ than the rotational period of the star, but one needs
to be careful with fast rotators and the spots that evolve more frequently than
that.}
...
\begin{itemize}
    ...
    constraining the period, this prior works in log-space\chdeleted[id=W]{to
    ensure the period remains positive and to make the distribution more
    symmetric}. The \chdeleted[id=W]{actual} prior is a uniform distribution in
    log-space, bounded by the log of the true value plus or minus some fraction.
    \chdeleted[id=W]{The final period is determined by exponentiating the result
    of this log-uniform distribution, which effectively creates a kind of
    log-uniform prior on the period itself.}
...
infer almost all five parameters within $\sim 2$ standard deviations. The only
parameters that are not constrained too well are planetary eccentricity $e$ and
the radius of spots $\mathbb{r}$, but they are still fairly close to the true
values: the true value for the eccenticity is 0.2 while the inferred median
value is 0.225, and the true value for the radius of spots is $10^\circ$ while
the inferred median value is $14.3^\circ$. \chcomment[id=W]{I think this
discussion is too pessisimistic; I look at Figure 7, and I see good recovery of
the true parameters, and no need to hedge by the ``close'' discussion above.}
...
\rho_{\odot}), \sigma_{\rho_{\rm RV}} / \rho_{\rm RV})$. This prior incorporates
observational constraints on the stellar density while properly accounting for
the uncertainty. The radius of the star stays fixed at 1 $R_\odot$.
\chcomment[id=W]{Do you want to include more discussion about how we are
effectively sampling the stellar density?  (This is well-known in the transit
community, so not strictly necessary, but the use of ``density'' here comes a
bit out of the blue!)}
...
The posterior distribution of our model \chadded[id=W]{applied to this data set}
...
inherent to transit modeling.  \chcomment[id=W]{We were doing ``transit
modeling'' above, and yet there was no multimodality there---what's different
here?  Is it the quality of the data?  Are we using a broad prior here, and
narrow above?  Something else?} First, the degeneracy between impact parameter
...
modes\chdeleted[id=W]{due to the momentum-based dynamics that govern their
exploration}.

To overcome these limitations, we utilized Parallel Tempering (PT), also known
as replica exchange MCMC.  \chcomment[id=W]{TODO: add references here.  See Earl
\& Deem 2005, and others from the \texttt{ptemcee} paper.} This method
...
8 temperatures \chdeleted[id=W]{geometrically spaced between $T=1$ and
$T=\infty$, with exchange attempts proposed every 10 iterations}.  A key feature
...
\chdeleted[id=W,comment={This doesn't really validate the sampling; if you like,
add something about autocorrelation lengths to monitor convergence.}]{To
validate adequate sampling across modes, we monitored the exchange acceptance
rates between adjacent temperature levels, aiming for rates between 10\% and
40\%.}

\subsection{Results}

\chcomment[id=W]{Did we say that we are using a non-evolving surface model here?  I've lost the thread.  Can we be clear?  If time-evolving, say how many epochs, etc; if not, note that this could be a weakness, particularly since we see evolution in the transits (i.e. we would infer a strange orientation so that the constant map can produce transit-to-transit variation because the transit occults different parts of the map across different transits).}
...
\begin{figure*}[hbt!]
    ...
    \chcomment[id=W]{I hesitate to ask, but these chains look really ratty.
    What is the autocorrelation length for the various parameters?  Can you run
    them longer so that the distributions become smoother and we are more
    confident that they have converged?}}
...
TOI-3884.  \chcomment[id=W]{Citation needed?}
%
\begin{figure*}[hbt!]
    \centering
    ...
    hemisphere of spot locations from transit data alone.  \chcomment[id=W]{This
    posterior seems to be quite confident that there are \emph{no} equatorial
    spots.  Is that simply a consequence of our assumption that the spots follow
    a Beta distribution (i.e. that the distribution is single-peaked in each
    hemishpere)?  Do we expect to have any information about the equator, given
    the orientations we are finding?}}
...
We find a planetary radius of $R_p = 0.188 \pm 3\times10^{-3}$ $R_\star$
\chcomment[id=W]{can we write out $\pm 0.003$ rather than $\pm 3 \times
10^{-3}$?} and an orbital period of $P_\text{orb} =
...
from eight separate transit events. Each panel shows the phase-folded
\chcomment[id=W]{what does it mean ``phase folded?''} photometric data (gray
...
\chcomment[id=W]{This implies that the spot is not a uniform polar cap OR that
it evolves in time, right?  Since there is variability?  If that is correct,
let's say so more explicitly.} Our model successfully captures the essential
...
\begin{figure*}[hbt!]
    ...
    lines indicate independent \chadded[id=W]{1-$\sigma$} constraints from
...
simply cannot resolve them.  \chcomment[id=W]{Citation needed?}
...
\chadded[id=W]{and Gravitational Wave Astronomy Group} at Flatiron Institute for
