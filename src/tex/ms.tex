...
There are two other assumptions to note here. First, the prior we place on each of the maps $\pmb{y(t)}$ is a fresh draw from 
the \texttt{StarryProcess}. Second, the linear interpolation approach is sensible if the $\Delta t$ between the fitted maps is 
small enough. In Section \ref{sec:experiment2}, we assume the spot time evolution is $\sim P_{\star}$. In general, we recommend
to have the chunks of the light curve span the time that's $\leq$ than the rotational period of the star, but one needs to be careful
with fast rotators and the spots that evolve more frequently than that.
