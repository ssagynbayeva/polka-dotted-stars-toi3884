...
for a smooth \chcomment[id=W]{``smooth'' is a term of art, and usually means $C^1$, i.e.\ once-differentiable.  Our interpolant is not smooth in this sense, at least at the nodes.  Maybe say evolves ``continuously''?} evolution of the maps over time. We assume that the surface map transitions smoothly from one epoch to the next, with the spherical harmonic 
...
\chcomment[id=W]{We need to explain the prior we place on each of the maps is a fresh draw from the \texttt{StarryProcess}, which is a pretty strong assumption; but seems to work OK.}
